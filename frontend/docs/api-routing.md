...
Production frontend builds resolve backend traffic against the current site origin instead of a direct Cloud Run hostname. `firebase.json` therefore rewrites the backend-owned same-origin families to Cloud Run for `/api/**` and `/detect-fields/**` (plus their exact-root variants), while the SPA-owned browser routes (`/forms`, `/forms/:slug`, `/upload`, `/ui`, `/ui/profile`, `/ui/forms/:formId`, `/ui/groups/:groupId`, `/respond/:token`, `/sign/:token`, `/verify-signing/:token`, and the account-action handlers) are rewritten to `app-shell.html`. There is intentionally no catch-all `** -> /app-shell.html` rewrite, because unknown public URLs must fall through to Hosting `404.html` instead of becoming soft 404 homepage responses. The Fill By Link -> Sign relationship still uses that same route pair, but the transition is now email-based: respondents begin on `/respond/:token`, the backend stores the response and emails the signer, and the signer later opens `/sign/:token` from the mailbox they control.

The public form catalog asset binaries do not go through Cloud Run. The generated catalog dataset stores relative PDF and thumbnail paths, and the runtime resolves them against `VITE_FORM_CATALOG_ASSET_BASE`. In local dev that remains `/form-catalog-assets` via the Vite middleware; in prod it should point at the public GCS bucket that `scripts/deploy-form-catalog-assets.sh` manages.

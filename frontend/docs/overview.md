...
- Search & Fill writes source data into number inputs but leaves calculated outputs/intermediates untouched. The backend recomputes those values during materialization so Search & Fill, Fill By Link, API Fill, editable download, flat download, and signing source freeze use the same calculation result.
...
- Number inputs publish as respondent-facing questions. Calculated outputs and calculated intermediates are omitted from the public question list because DullyPDF computes them server-side from submitted answers.
...
- Public Fill By Link and usage-docs copy recommends flat outputs for external recipients, respondent receipts, signed packets, and final records because editable field styling can vary across mobile, browser, and email-preview PDF viewers.
- Calculation fields follow the same viewer guidance: DullyPDF precomputes visible values before every output, while editable-PDF live recalculation is primarily for Adobe Acrobat/Reader. Browser and mobile viewers may preserve the precomputed value without running the generated calculation JavaScript.

...
- `http://localhost:5173/usage-docs` and child `/usage-docs/*` routes for English public documentation copy.
- `http://localhost:5173/es/usage-docs` and child `/es/usage-docs/*` routes for Spanish public documentation copy.
...
- Envelope routing modes now behave differently on purpose: `Separate` creates one independent request per signer, `Parallel` sends one shared envelope to every signer at once, and `Sequential` only emails the active signer. Later sequential recipients stay queued with `Waiting for turn` UI until the previous signer completes, and the public completion page keeps final signed-PDF / audit downloads locked until the full envelope finishes.

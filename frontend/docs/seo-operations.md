...
- The prerendered shell should include semantic body copy, contextual internal links, and route-specific headings before React loads.
- The generated HTML now renders that copy in `#seo-static-shell` ahead of `#root`; once the React route mounts, the client removes the prerendered shell so the interactive page takes over without a duplicate-content flash.
...
- Confirm prerendered route HTML contains `data-seo-shell-visible="true"` and does not contain `display:none`.

...
describe('usageDocsContent route resolver', () => {
  it('resolves canonical English /usage-docs routes', () => {
  ...
  it('resolves canonical /es/usage-docs routes', () => {
    expect(resolveUsageDocsPath('/es/usage-docs')).toEqual({ kind: 'canonical', pageKey: 'index', locale: 'es' });
    expect(resolveUsageDocsPath('/es/usage-docs/search-fill')).toEqual({ kind: 'canonical', pageKey: 'search-fill', locale: 'es' });
    expect(resolveUsageDocsPath('/es/usage-docs/fill-by-link')).toEqual({ kind: 'canonical', pageKey: 'fill-by-link', locale: 'es' });
    expect(resolveUsageDocsPath('/es/usage-docs/signature-workflow')).toEqual({ kind: 'canonical', pageKey: 'signature-workflow', locale: 'es' });
    expect(resolveUsageDocsPath('/es/usage-docs/api-fill')).toEqual({ kind: 'canonical', pageKey: 'api-fill', locale: 'es' });
    expect(resolveUsageDocsPath('/es/usage-docs/search-fill/')).toEqual({ kind: 'canonical', pageKey: 'search-fill', locale: 'es' });
  });
  ...
  it('builds canonical usage-docs hrefs', () => {
    ...
    expect(usageDocsHref('index', 'es')).toBe('/es/usage-docs');
    expect(usageDocsHref('detection', 'es')).toBe('/es/usage-docs/detection');
  });
});

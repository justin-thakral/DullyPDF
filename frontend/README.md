...
Public product usage documentation is available at canonical `/es/usage-docs/*` URLs. Legacy `/docs/*` URLs permanently redirect to matching canonical docs routes.
Indexable localized public routes now include the India `/in` cluster and the Spanish `/es` cluster. India covers `/in`, twenty India workflow/industry intent pages, `/in/blog`, and ten India PDF automation guides. Spanish covers `/es`, ten workflow-library pages, ten industry-solution pages, `/es/blog`, ten Spanish blog guides, and the Spanish hubs at `/es/flujos-de-trabajo` and `/es/industrias`. Legacy global English intent-page records remain available to the route data layer for compatibility, but they are marked low-value/noindex and excluded from sitemap generation.

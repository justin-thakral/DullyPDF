...
- Detection status: `GET /detect-fields/{sessionId}` returns status + fields when ready, and now also includes session-backed rule metadata needed for frontend restore.
...
- Fill By Link: `GET /api/fill-links`, `POST /api/fill-links`, `PATCH /api/fill-links/{id}`, `POST /api/fill-links/{id}/close`, and public `/api/fill-links/public/*` routes (supports one link per saved template or one merged link per open group). Published links now persist a normalized `web_form_config` plus a published question schema so owners can control global defaults, per-question requiredness, per-question text limits, and template-only custom web-form questions. Plan enforcement no longer caps the number of active links; instead, accepted submissions consume an account-level monthly response quota (`25` on base, `10,000` on premium) while each link keeps lifetime `response_count` analytics for reporting only. Template links can optionally freeze a publish snapshot so accepted respondents can download a PDF copy of their own submission later via a public response download route. Respondent downloads default to a flattened `flat` artifact, while template owners can opt into an `editable` respondent download mode that preserves widgets for that public response download without affecting signing artifacts. If post-submit signing is enabled, the publish/update routes force respondent downloads back to `flat` so the public response artifact never stays editable. Template links can also enable a post-submit signing handoff: the owner maps signer name/email questions at publish time, the publish flow requires a visible email question for the signer-email mapping, requires an explicit U.S. e-sign eligibility attestation for the selected document category, and consumer-mode post-submit signing now also requires request-specific paper-copy, fee, withdrawal, and contact-update disclosures before publish succeeds. Owners can also flag the post-submit request as company-binding, which makes the signer provide a representative title, company name, and explicit authority attestation during the final signing action. DullyPDF records that authority attestation in the retained audit record but does not independently verify corporate authority. The public submit route validates that signer name/email before it stores the response. Once validated, the backend materializes the immutable filled PDF from the stored response snapshot, creates a signing request, and sends the signer a continue-signing email for that exact record instead of returning a live `/sign/:token` URL to the submitting browser. Public `/api/fill-links/public/{token}/retry-signing` now resends that signing email against the stored response record with delivery-state feedback and cooldowns, while already-sent/completed retained requests can be reused without rematerializing the deleted source template again. When a base-plan template lock or saved-form deletion makes the source template unavailable for a draft/new request, the public signing payload now returns a non-retryable sender-contact error instead of pretending the respondent can fix it with another retry. If the signing category policy tightens later, the response-to-signing handoff revalidates the stored category before creating or reusing a request, so stale published configs cannot bypass newly blocked document classes. Owner response listings now embed linked signing invite state plus signed-PDF / audit-receipt download paths when a response has completed signing.
- Signing workflow: `GET /api/signing/options`, `GET /api/signing/requests`, `POST /api/signing/requests`, `GET /api/signing/requests/{id}`, `GET /api/signing/requests/{id}/artifacts`, `GET /api/signing/requests/{id}/artifacts/{artifactKey}`, `POST /api/signing/requests/{id}/send`, `POST /api/signing/requests/{id}/revoke`, `POST /api/signing/requests/{id}/reissue`, `GET /api/signing/public/validation/{token}`, and public `/api/signing/public/*` routes. The owner workflow remains one-request-per-signer, but the frontend can now create and send recipient batches by saving multiple drafts from pasted/uploaded TXT or CSV data, then tracking them through the owner `Responses` view. Draft creation now requires an explicit U.S. e-sign eligibility attestation for the selected document category, and consumer-mode drafts also require request-specific paper-copy, fee, withdrawal, and contact-update disclosures before the draft can be saved. Owners can additionally flag a request as company-binding; when they do, the final signer step requires a representative title, company name, and explicit authority attestation. DullyPDF records that attestation text/version/hash and the signer-supplied representative fields in the retained audit evidence, but it does not independently verify corporate authority. Base-plan template access rules now also apply to signing source templates: if a saved form falls outside the oldest-created accessible set after downgrade, owners cannot create or send signing drafts against that locked form until they reactivate Premium. Already-sent or completed signing records stay retained, while drafts tied to a deleted source form are still invalidated. Send/reissue revalidate the current blocked-category policy so stale drafts cannot bypass later compliance-policy changes. Signing now uses an account-level monthly send quota by plan: the free tier defaults to 25 sent signing requests per UTC month, Pro defaults to 10,000, saving or revoking an unsent draft does not consume quota, the first successful send does, and resending an already-sent request does not consume quota again. Sent requests now receive a request-level expiry window, the owner can revoke any unsatisfied draft/link from the workspace, and reissue rotates a versioned public signer URL so previous tokens stop resolving after replacement. Signing records now persist explicit signer transport metadata (`signer_contact_method`, `signer_auth_method`) so invite delivery policy, ceremony gating, and downstream webhook payloads do not need to infer channel/auth behavior from legacy source-type heuristics. Consumer-mode source previews stay locked until e-consent is recorded, and public ceremony sessions are bound to the bootstrap client fingerprint instead of behaving like replayable bearer tokens. Email OTP verification now protects every emailed signing request source in the default config, so `/sign/:token` must pass `/verification/send` and `/verification/verify` before the immutable PDF, review, consent, manual fallback, signature adoption, or completion routes are available. Owner-sent source artifacts are now normalized into flattened non-editable PDFs before the backend hashes and stores them, so the retained "immutable source" copy does not reopen with live form widgets in standard viewers. The signature-adoption step now persists four signer-controlled render modes for the visible signature mark: typed name, default legal name, drawn signature, and uploaded signature image. The same bound session gate now protects `/document` and public artifact downloads too, including already-completed requests, but the completion payload no longer exposes live artifact URLs. Public pages must first mint a short-lived artifact link through `POST /api/signing/public/{token}/artifacts/{artifactKey}/issue`, then redeem it through `GET /api/signing/public/artifacts/{artifactToken}` with the bound signing session header, so completed links no longer act like long-lived bearer URLs for signed PDFs or audit receipts. Missing source/artifact blobs on those public download/completion paths now fail with deterministic client errors instead of surfacing generic `500`s. Consumer requests still expose a separate `/consumer-access-pdf` access-check route, require the signer to enter the access code from that PDF before `/consent` succeeds, and allow explicit `/withdraw-consent` before completion so the audit trail retains stronger E-SIGN-style consumer evidence. The signer-facing public payload now also exposes sender name/contact and removes on-page manual-fallback language when that button is disabled, so the disclosure package matches the actual ceremony options. Phase 4 now persists the exact server-defined consumer disclosure payload, its SHA-256 digest, the first presented timestamp, the consent scope, and the recorded access-demonstration method/result on the request itself, then seals that evidence into the audit manifest and summarizes it in the audit receipt. Milestone 4 extends `Sign` mode so the final signer action now materializes a flattened signed PDF, stores an audit-manifest JSON envelope plus a human-readable audit receipt PDF, and exposes signed-PDF / audit-receipt downloads to the public completion page while owners can retrieve the full artifact set later. Completed owner responses now also expose a one-click dispute-package export that bundles the retained source PDF, signed PDF, audit receipt, owner audit manifest envelope, validation snapshot, and invite/delivery metadata. Completed requests also mint a stable `/verify-signing/:token` validation path, the audit receipt now embeds that validation URL in both text and QR form, and the public validation route verifies the retained audit envelope plus signed-PDF hashes without requiring a separate signing session. When a PDF signing identity is configured, completion also embeds a cryptographic PDF signature directly into the finalized PDF, can attach an RFC 3161 timestamp via `SIGNING_PDF_TSA_URL`, and surfaces that embedded-signature verification result through the same public validation route. Best-effort outbound signing webhooks can now fan out lifecycle events such as request creation, invite delivery, review/consent, completion, revoke, and reissue to server-configured URLs without making the core signing state transition fail if delivery is unavailable. The public receipt intentionally redacts signer email/IP/user-agent fields; the owner-only audit manifest retains the full evidence set. That owner audit evidence is what DullyPDF expects the sender to rely on in disputes: request/document identifiers, immutable source and signed hashes, sender/signer identity fields, invite delivery metadata, OTP/access-check state, ceremony timestamps, signature-adoption details, digital-signature metadata, retained artifact paths, company-authority attestation text/version/hash plus representative title/company fields when that track is enabled, and consumer disclosure payload/hash plus access-demonstration evidence when consumer mode is used. This is intended to line up with the core E-SIGN mechanics in 15 U.S.C. § 7001: signer intent, logical association of the signature with the exact record, consumer disclosures and affirmative consent where required, and retention in a form that can be accurately reproduced later. Phase 5 adds a storage-tier split for signing: source/completion uploads stage under short-lived storage first, finalized artifacts live under the dedicated `SIGNING_BUCKET`, and deploy preflight now validates that the final bucket exposes a retention-capable policy before prod deploys continue. Milestone 5 also brings `Fill and Sign` onto the same immutable boundary: the frontend freezes reviewed workspace values through `/api/forms/materialize`, owner drafts can persist Fill By Link response provenance (`source_type=fill_link_response`, `source_id`, `source_link_id`, `source_record_label`), and the send route now requires an explicit owner-review confirmation before a reviewed fill can be frozen and sent. When a Fill By Link response already spawned a signing request, deleting the parent Fill By Link now preserves that response document as retained provenance instead of hard-deleting it, and the signing audit manifest seals the source link id plus source record label so the retained signing record does not depend on the Fill By Link row surviving forever. Invite delivery is best-effort per request, and provenance/audit event persistence is best-effort too, so successful create/send/retry transitions do not fail only because telemetry storage is temporarily unavailable. When Gmail delivery is unavailable outside production the backend marks the request for manual follow-up instead of failing the send. The signing stack is engineered for ordinary U.S. business records plus the repo-defined consumer-consent ceremony, but it is not a blanket legal determination for every document class or jurisdiction. Excluded categories, regulated workflows, notarial acts, and document-type-specific consumer rules still require separate policy/legal review before enabling them in production.
- Signing compliance note: DullyPDF does not auto-classify source PDFs into legal document categories. The sender or owner selects the document category, confirms the eligibility attestation, and remains responsible for choosing a supported category instead of routing excluded court, family-law, UCC-excluded, recall/safety, primary-residence notice, or similar records through the product. This is a documented product limitation, not an automated legal classifier. Consumer mode is the stronger ceremony for records that trigger consumer electronic-disclosure rules, while business mode is the lighter ordinary-business path. Company-binding mode records only the signer’s authority attestation; it is not independent proof that the signer actually had authority to bind the entity. Notarial and acknowledgment workflows remain intentionally out of scope in DullyPDF v1 even though some jurisdictions may allow separate electronic-notary programs.
- Development-signature note: outside production, DullyPDF can fall back to a bundled self-signed PDF signing certificate when no PKCS#12 or Cloud KMS signing identity is configured. PDF viewers may report that certificate as untrusted because it is not chained to a public CA, but the embedded signature still provides cryptographic tamper evidence over the finalized PDF. In that mode, the retained audit log, audit receipt, immutable source/signed artifacts, and `/verify-signing/:token` validation record are the intended verification path. That can still be operationally sufficient for many internal or ordinary-business workflows, but it is not equivalent to a publicly trusted production signing identity or timestamped trust service.
...
- Session PDF restore: `GET /api/sessions/{sessionId}/pdf` (auth required; returns the original session PDF bytes so `/ui` workspaces can recover after reload while the session is alive).
- PDF page tools: `POST /api/pdf/page-tools` (auth required; rewrites the active source PDF from a staged final page list, supporting current-page reorder/delete, 90-degree rotations, and inserted pages from uploaded PDFs while enforcing fillable page limits; existing current-source widget annotations are reattached into the output `/AcroForm`, while inserted-PDF widgets are stripped because the editor does not import metadata for inserted pages).
- PDF optimize: `POST /api/pdf/optimize` (auth required; applies lossless PyMuPDF cleanup, object-stream rewriting, and stream/font/image deflate to reduce file size without intentionally lowering image quality).
- Materialize fillable: `POST /api/forms/materialize` (auth required; validates and evaluates DullyPDF calculation fields, injects fields into a PDF upload, enforces fillable page limits, keeps DullyPDF-only image/barcode helpers as marker text fields in editable exports, and stamps those helpers only in flat exports; editable helper markers can carry a compact `DULLYPDF_META:` setup line for image paths, source-field dependencies, and barcode classes, while flat exports strip marker widgets before flattening so marker codes are not baked into page text; backend-generated 1D barcode helpers use the same 9-digit Code 128 aspect as the editor). This endpoint stays available for save/signing/internal materialization and is not the public generated-PDF quota boundary.
- Quota-enforced generated PDF downloads: `POST /api/forms/download` for a single workspace PDF and `POST /api/forms/group-download` for group ZIP exports (auth required; base users default to `25` generated PDFs per UTC month, Pro/God are unlimited by default, group ZIPs count one quota unit per PDF inside the archive, and browser retries must pass an idempotent `downloadRequestId`; quota failures return `429` with detail code `pdf_download_limit_reached`).
- Profile summary: `GET /api/profile` (tier info, credits, limits, generated-PDF download usage, billing metadata, Stripe payment-recovery state, and template-access lock state when applicable).
- Downgrade retention compatibility routes: `PATCH /api/profile/downgrade-retention` and `POST /api/profile/downgrade-retention/delete-now` remain for existing clients, but under the lock-based policy they no longer delete saved forms. The summary now reports the deterministic oldest-created accessible set plus the locked remainder.
...
- Billing portal: `POST /api/billing/portal-session` (auth required; returns a Stripe-hosted Customer Portal URL so subscribed users can update payment methods without DullyPDF handling card data).
...
- `SIGNING_AUDIT_KMS_KEY` (required in production for audit-manifest signing; may point at a CryptoKey or a specific CryptoKeyVersion. When a CryptoKey is provided, DullyPDF resolves the newest enabled CryptoKeyVersion because Cloud KMS asymmetric signing keys do not expose a primary version.)
...
- `SIGNING_PDF_KMS_KEY` + `SIGNING_PDF_CERT_PEM` / `SIGNING_PDF_CERT_PEM_BASE64` / `SIGNING_PDF_CERT_PATH` (optional Cloud KMS + X.509 certificate identity for embedded PDF signatures; `SIGNING_PDF_KMS_KEY` may point at a CryptoKey or a specific CryptoKeyVersion, and CryptoKey references resolve the newest enabled version)
...
- `SIGNING_PDF_USE_BUNDLED_DEV_CERT` (dev/test-only override for the repo's local test certificate; outside production the bundled certificate is used by default when no PKCS#12 or KMS identity is configured, local validation trusts that exact self-signed cert to avoid false untrusted warnings in dev, and this flag can be set to `false` to disable that fallback explicitly)
...
- `SANDBOX_SIGNING_REQUESTS_MONTHLY_MAX_BASE` / `SANDBOX_SIGNING_REQUESTS_MONTHLY_MAX_PRO` / `SANDBOX_SIGNING_REQUESTS_MONTHLY_MAX_GOD` (monthly sent-signing-request quotas; defaults 25 / 10000 / 100000)
- `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_BASE` / `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_PRO` / `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_GOD` (monthly generated-PDF download quotas; defaults 25 / unlimited / unlimited, where `0` or unset means unlimited for Pro/God)
...
- `SANDBOX_DETECT_MAX_PAGES_BASE`, `SANDBOX_DETECT_MAX_PAGES_PRO`, `SANDBOX_DETECT_MAX_PAGES_GOD`
- `SANDBOX_FILLABLE_MAX_PAGES_BASE`, `SANDBOX_FILLABLE_MAX_PAGES_PRO`, `SANDBOX_FILLABLE_MAX_PAGES_GOD`
- `SANDBOX_SAVED_FORMS_MAX_BASE`, `SANDBOX_SAVED_FORMS_MAX_PRO`, `SANDBOX_SAVED_FORMS_MAX_GOD`
- `SANDBOX_FILL_LINK_RESPONSES_MONTHLY_MAX_BASE`, `SANDBOX_FILL_LINK_RESPONSES_MONTHLY_MAX_PRO`, `SANDBOX_FILL_LINK_RESPONSES_MONTHLY_MAX_GOD`
- `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_BASE`, `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_PRO`, `SANDBOX_PDF_DOWNLOADS_MONTHLY_MAX_GOD`
...
- Base users now refill to a 10-credit monthly floor instead of receiving a one-time lifetime starter pack. On each UTC month boundary, the backend tops a base account back up to `10` only when the current balance is below that threshold; balances above `10` are left unchanged. Credits are billed per page bucket using server-side page counts:
...
- `GET /api/profile` now includes `creditPricing` (server bucket/base-cost settings), generated-PDF download quota usage (`pdfDownloadsThisMonth`, workspace-vs-group split counts, successful download batches, `pdfDownloadsRemaining`, `pdfDownloadUsageMonth`, `pdfDownloadResetAt`, plus `limits.pdfDownloadsMonthlyMax`), `billing` metadata (`enabled`, plan catalog, subscription linkage/status, and cancellation schedule fields), and a compatibility `retention` payload that now describes lock-based access instead of a grace/purge queue.
- User-triggered workspace PDF downloads use Firestore-backed quota accounting in `pdf_download_usage_counters`, `pdf_download_events`, and `pdf_download_request_guards`. The request guard makes single-PDF and group-ZIP downloads idempotent so network retries do not double-charge monthly quota.
- On downgrade, the backend keeps the oldest saved forms up to the base limit accessible and marks the rest locked in place. Locked templates and their related records are preserved instead of being purged, dependent Fill By Link and signing draft/new flows are blocked until the owner upgrades again, and downgrade-managed links reopen automatically when the source template becomes accessible again.
...
- Fill By Link response validation is schema-driven: published questions can carry per-question `required`, `maxLength`, placeholders, help text, option metadata, and calculation number-input metadata. Calculation number inputs are normalized as integer answers and rejected before response storage when a respondent submits decimals or non-numeric values, which keeps later respondent PDF downloads and signing materialization from failing on invalid numeric source data. The existing environment-level answer caps remain the hard safety ceiling on stored submissions.
- DullyPDF calculation fields are server-materialized before PDF generation. Number inputs are integer-only in v1; calculated outputs/intermediates are validated for missing dependencies and cycles, evaluated with deterministic decimal handling, forced read-only, and written as normal text-field values. Editable exports additionally receive generated Acrobat JavaScript, `/AcroForm /CO` dependency order, and `/DullyPDFCalculations` metadata for DullyPDF re-import, but flat exports and signing source artifacts rely on the server-computed value rather than viewer-side JavaScript.
...
The smoke helper refuses non-local base URLs by default because it sends valid signed
webhook events that can mutate billing state. Use `--allow-non-local` only for an
isolated staging backend, for example:

```bash
./scripts/test-billing-webhooks.sh env/backend.dev.env https://staging.example.com --allow-non-local
```
...
- Checkout sessions pass `STRIPE_CHECKOUT_DISPLAY_NAME` and optional `STRIPE_CHECKOUT_BRAND_ICON_FILE` through Stripe `branding_settings` so the hosted Stripe page uses DullyPDF branding even when account-level branding must be managed in the Stripe Dashboard.
...
- `invoice.payment_failed` records payment-recovery metadata and leaves renewal failures `past_due` so Stripe Smart Retries can recover the payment before DullyPDF downgrades the account. Initial subscription-create failures are recorded as `incomplete` so a failed first checkout does not look like an active recovering renewal.
- `invoice.updated` refreshes existing payment-recovery metadata, including retry timestamps sent by Stripe Automations, without changing role, credits, or downgrade state.
- `invoice.paid` and active/trialing subscription updates clear stale payment-recovery state; only terminal subscription states such as `canceled`, `incomplete_expired`, and `unpaid` downgrade the user to Base and apply the lock-based retention policy. Non-active transitional statuses are recorded without a local downgrade, and `incomplete_expired` does not apply retention when Pro access was never active.
- Invoice webhooks resolve subscription, user metadata, and price ids from both legacy invoice fields and Stripe's newer `parent.subscription_details` / `lines.data[].pricing.price_details` payload shape.
- Webhook handlers skip stale invoice, checkout, and subscription lifecycle events when a newer payment recovery or a different active local subscription is already stored.
- `STRIPE_PAYMENT_RECOVERY_WINDOW_DAYS` controls the local recovery deadline shown in Profile. It does not independently downgrade users; Stripe must still move the subscription to `canceled`, `incomplete_expired`, or `unpaid` through webhook delivery.
- `STRIPE_BILLING_PORTAL_CONFIGURATION_ID` optionally pins hosted Billing Portal sessions to a Stripe portal configuration. Production currently uses `bpc_1TZKUUFJ44ExkRGSxwDSlRYS`, which enables payment-method updates and invoice history. Portal sessions use Stripe's direct `payment_method_update` flow and return to Profile with `billing=payment-method-updated`. On return, the frontend calls `POST /api/billing/payment-method-recovery/sync`; the backend copies Stripe's customer-level default payment method onto the recovering subscription, retries the latest open invoice, and lets normal Stripe webhooks heal or finalize local billing state. Set `STRIPE_BILLING_PORTAL_RETURN_URL` to override the return target; otherwise the backend derives `/ui/profile` from `STRIPE_CHECKOUT_SUCCESS_URL`.

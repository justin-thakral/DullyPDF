...
- Materialize fillable: `POST /api/forms/materialize` (auth required; validates and evaluates DullyPDF calculation fields, injects fields into a PDF upload, and enforces fillable page limits).
- Profile summary: `GET /api/profile` (tier info, credits, limits, billing metadata, and template-access lock state when applicable).
...
- `GET /api/profile` now includes `creditPricing` (server bucket/base-cost settings), `billing` metadata (`enabled`, plan catalog, subscription linkage/status, and cancellation schedule fields), and a compatibility `retention` payload that now describes lock-based access instead of a grace/purge queue.

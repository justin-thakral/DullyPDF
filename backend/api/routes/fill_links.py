...
from backend.services.signing_dispute_package_service import owner_dispute_package_available
...
def _sync_fill_link_retention(user_id: str) -> None:
    sync_user_downgrade_retention(user_id, create_if_missing=True)


def _serialize_link(record, *, monthly_response_limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "accountMonthlyResponsesMax": monthly_response_limit,
        ...
        "canAcceptResponses": record.status == "active",
...
def _ensure_fill_link_templates_available(user, template_ids: List[str]) -> None:
    locked_template_ids = get_user_retention_pending_template_ids(user.app_user_id)
    if not locked_template_ids:
        return
    if any(template_id in locked_template_ids for template_id in template_ids):
        raise HTTPException(
            status_code=409,
            detail=(
                "This Fill By Link cannot be published because one or more saved forms are locked on the base "
                "plan. Upgrade to reactivate that saved form before publishing or reopening the link."
            ),
        )
...
def _fill_link_scope_conflict_detail(closed_reason: Optional[str]) -> str:
    ...
    if normalized_reason in {"downgrade_retention", "template_access_locked"}:
        return "This Fill By Link can no longer be activated because one or more source saved forms are locked on the current plan."
...
async def list_owner_fill_links(
    ...
    monthly_response_limit = resolve_fill_link_responses_monthly_limit(user.role)
    ...
    return {
        "links": [
            _serialize_link(close_fill_link_if_scope_invalid(link) or link, monthly_response_limit=monthly_response_limit)
            for link in links
        ]
    }
...
async def create_owner_fill_link(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {"success": True, "link": _serialize_link(record, monthly_response_limit=monthly_response_limit)}
...
async def update_owner_fill_link(
    ...
    if not updated:
        raise HTTPException(status_code=404, detail="Fill By Link not found")
    return {"success": True, "link": _serialize_link(updated, monthly_response_limit=monthly_response_limit)}
